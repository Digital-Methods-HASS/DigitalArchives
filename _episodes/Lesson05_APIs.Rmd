...
Let's navigate to [Trove API Console](http://troveconsole.herokuapp.com/). Please beware that the actual interface might look somewhat different than the screenshots below due to recent UI changes:
...
2. Look at the first example and try to decipher what the Parameters mean.  Zone is a category where the search is running, such as 'newspapers', or 'books'. In this case, we are looking everywhere in Trove. Wragge is an Australian surname, 'n' is a number of records you will be shown, and encoding is the format that Trove will return.
...
4. Copy the first query under 'Search Newspapers'. It looks something like https://api.trove.nla.gov.au/v2/result?q=wragge+AND+weather&zone=newspaper&encoding=json designed to look for the terms wragge and weather, in newspapers with no limit on number of records. 
