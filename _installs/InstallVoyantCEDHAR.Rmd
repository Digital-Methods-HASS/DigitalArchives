...
In order to try out the Introduction to Archives and Digital Methods case study on candle distribution in parishes around Aarhus during World War II in the CEDHAR workshop, you need to have access to the *Aarhus parish minutes dataset*,a Danish stopword list, and the *Voyant server* desktop software or a cloud instantiation.  If you only wish to try Voyant out as part of the case study, you can use the link to online Voyant provided via email and stop reading now.
If you wish to use Voyant offline in order to explore texts that are private, unpublished, sensitive, or otherwise encumbered, follow these instructions to install the desktop version before the workshop. If you encounter errors while installing, please document them (with screenshots) and alert one of the instructors at the start of the workshop. 

## Software and Data needed

Read here about the software and go into the Installation section to install and configure it. 

* [JAVA](https://www.oracle.com/java/technologies/javase-jre8-downloads.html): Before downloading Voyant, you will need to download Java if you do not already have it. If you do have Java installed, you should still download and update your version.  For an update, download the **64-bit** offline version (recommended) for [Windows here](https://javadl.oracle.com/webapps/download/AutoDL?BundleId=242990_a4634525489241b9a9e1aa73d9e118e6), and for [Mac here](https://javadl.oracle.com/webapps/download/AutoDL?BundleId=242981_a4634525489241b9a9e1aa73d9e118e6). If you have difficulty getting the right Java, please sign up for the Help Desk.
...
* [VOYANT](https://github.com/sgsinclair/VoyantServer/releases/tag/2.4.0-M45): This is a rather large download [~450Mb], requiring solid internet connection. Remember to scroll down this webpage and click on the `VoyantServer2_4-M45.zip` file to download.


* PARISH MINUTES DATASET has been provided as a zipped file via email.  Please read the [associated handout](https://drive.google.com/file/d/1t6Zt2iKak1y--vc07nwzy0i9Ul2CK1kS/view?usp=sharing) to learn about the process of its creation and curation. The Aarhus City Archive parish minutes contain the entirety of the minutes filed during 1939-1945 which are now in public domain. 

## Installation of Voyant Desktop

Although Voyant Tools is a web-based set of tools, we will be downloading it and running it locally. This way you can work offline, should you wish to analyze your texts without an internet connection. You also avoid disseminating the datasets online and overloading the web servers. Furthermore, using Voyant locally means that your project will not be cached and stored on the Voyant web-server, and you can restart the server if you encouter any problems.
...
4. Double click the `VoyantServer.jar` file. Your computer may mistrust the file so on a Mac you may need to "force" open the program by "right-clicking" and pressing "open". On Windows, you may likewise need to insist that you really wish to open the file. When you open this file a new window (see image below) will pop up with numerous server controls. If you are curious about what these all mean you can read about the options in the documentation for the Voyant Server. However, in short, this window begins to launch a server which will generate the Voyant tools interface.
...
** If your default browser is Chrome you will need to paste the address into a new window in a different browser. Both Safari and Firefox work. The default address should be: 
http://127.0.0.1:8888/
...
- Once you have selected the file, click *Open* and Voyant will upload the file and direct you to the dashboard. This may take a few seconds to a few minutes depending on your computer's performance and the memory you allocated to Voyant (and the size of the corpus of documents).  You may also see an error message pop up. Disregard it and close it as it is not fatal. 


Exploring the *Default Skin*
After uploading the corpus, you will be redirected to what Voyant calls the *default skin*, or, the default configuration of tools. To understand the many options, tools, and settings available and necessary to process  Danish texts in Voyant see [Adela's slides](https://github.com/Digital-Methods-HASS/DigitalArchives/blob/master/VoyantSlides.pdf) and [Voyant documentation](http://voyant-tools.org/docs/#!/guide).

# Managing Text and Visualisations in Voyant

We will cover these topics in the hands-on session. 

- creating `stopword` lists for non-default languages (e.g. Danish)
- searching concepts in Voyant
- exporting visualisations and frequency data from Voyant

The following sections serve as a guide to some of the computational analyses you can do on texts and as an illustration of the scholarly work that involved text analysis. Finally, there is a glossary of terms you might encounter during this course.


# Your Task

Your task today is to install Voyant and download the data before Thursday. We will introduce the research topics and provide further information on how to work with the data. Feel free to read the sections below if you are interested in knowing more about text analysis and different methods it entails.
...
[Americas Public Bible](https://americaspublicbible.org/), Lincoln Mullen
...
---
Credits:
This guide has been inspired by [Amanda Regan](https://www.amanda-regan.com/Workshop-UCR/) and tested and improved by Antonio Rivero Ostoic, Josephine Moeller Jensen, and Jonas Cardie.
