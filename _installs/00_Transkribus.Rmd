...
If you do not have "Administrator" rights, Windows will produce a warning message, such as: "Windows Protected Your Computer", etc. Do **not** press "Confirm", but go to "More Information" link. The next screen lets you approve  Transkribus by pressing "Run Anyway".
...
If you run the program the first time, it may not start because it is a non-signed application ("... can't be opened because it is from an unidentified developer" message).  In this case, right-click (or control-click) the application and choose "Open". In the appearing dialog box, click "Open" again (not 'Move to bin'):
