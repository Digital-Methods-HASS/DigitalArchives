...
- [JAVA](https://java.com/de/download/manual.jsp#win): Before downloading Transkribus, you will need to download the newest Java 8 if you don’t already have it. If you do have Java installed, you should still download and update your version, or at least [check it](https://java.com/en/download/help/version_manual.xml).  
...
2. Download the correct version (for your OS) for free from the website. If the download fails - please download the latest release from here: [alternative download location](https://transkribus.eu/TranskribusX/releases/) After download you should see a ZIP File in the download directory of your computer.
...
5 Notes for first launch

- Windows: If you do not have "Administrator" rights, Windows will produce a warning message, such as: Your Computer is Protected by Windows", etc. Do not confirm, but go to "More Information". There you can agree that this is not malware and that you want to run Transkribus on your computer.
...
- Linux: If the OS is (or is based on) Ubuntu 17.04, installing libwebkit is necessary:
```{r, eval=FALSE}
sudo apt install libwebkitgtk-1.0-0
```

If you encounter other problems and difficulties with installation: Have a look at Transkribus [FAQ page](https://transkribus.eu/wiki/index.php/Questions_and_Answers#Download_and_Installation)
...
## Segmenting and transcribing texts 
...
For those who prefer written instructions, segemntation and transcription are both covered in [How to use Transkribus - in 10 steps (or less) Guide](https://transkribus.eu/wiki/images/7/77/How_to_use_TRANSKRIBUS_-_10_steps.pdf) again focusing on steps 1-8 for a general overview of tools available.

Everyone: please also consult the [Transcription Guidelines](https://transkribus.eu/wiki/images/3/31/Transkribus_Transcription_Conventions.pdf) before you commence transcription. 

There are additional tips and tricks for special topics, such as [Transcribing Tables](https://transkribus.eu/wiki/images/1/14/HowToProcessTables.pdf), however, that does not apply in our case. 
...
1. Allocate different pages or documents from your case file to one another in your group and start transcribing them
2. Document any decisions, and consult any problems/questions you have regarding segmentation and transcription in your group. Document these, and group agreements and disagreements in a  Readme file (on class GoogleDrive). 
3. As you progress, save your work and gather question for consultation in class.
4. Consult your group issues in class and revise your transcriptions in light of the decisions
5. Submit a minimum of one page of segmented and transcribed text (after in class consultation) in a .txt file labelled with filename_groupnumber_lastname.txt  in your group GoogleDrive space.
