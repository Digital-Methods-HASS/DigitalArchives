...
In order to proceed with the transcription of your case files during Wednesday in week 36, you will need to  set up Transkribus on your computer. This is a multiple-step process where you:
...
Please ensure to have completed all steps before Wednesday in week 36. If you need help with installation, please alert course assistants by 10am Thursdayin week 35
...
- [TRANSKRIBUS](https://readcoop.eu/transkribus/wiki/download-and-installation/) is a comprehensive platform for transcription and searching of historical hand-written documents, which facilitates - with large enough transcribed sample - the automated recognition of the documents. Note there are two versions, the Expert Client standalone version and the Lite version. Follow the instructions to download the Expert standalove version  

## Transkribus Installation

1. Go to the [Getting started page](https://readcoop.eu/transkribus/start/) and Register. You can watch the Transkribus Expert Client video and then sign up for free at the Setting up your Account section. To register, you will need to provide your name, e-mail address, and password. Remember the credentials as you will need them to launch the software later. 

![](../images/transkribus-site-new.png)


2. Move to [Downloads and Installation](https://readcoop.eu/transkribus/wiki/download-and-installation/) and follow the instructions here to download the Expert Client software (for free via the blue button). After download you should see a ZIP File in the download directory of your computer, called Transkribus-1.16.1-package.zip or similar.
...
3.  Unzip/Extract the file and open the resulting Transkribus directory (some Macs will extract the files automatically upon a click so may directly see a Transkribus-1.16 directory with different files inside)

4.  Start Transkribus:

- **Windows**:  use *Transkribus.exe* file, also called "Application" in the Type column, with the following icon
```{r}
Friday
```


![](../images/transkribus-icon.png){width=300px}

If you do not have **Administrator** rights, Windows will produce a warning message, such as: *Windows Protected Your Computer*, etc. Do **not** press *Don't Run*, but go to *More Info* link. The next screen lets you approve Transkribus by pressing *Run Anyway*.

![](../images/W10_protect.png){width=400px} ![](../images/W10_run.png){width=400px} 



- **Mac OS - Apple**:
    - Right-click on the downloaded archive (with the Transkribus icon).
    - Choose "Show Package Contents"
    - Go to *Contents* -> *MacOS*
    - Click on the executable file called Transkribus and choose "Open"

If you run the program the first time, it may not start because it is a non-signed application ("... cannot be opened because it is from an unidentified developer" message).  In this case, right-click (or control-click) the application and choose "Open". In the appearing dialog box, click "Open" again (not 'Move to bin'):

![](../images/transkribus-open.png){width=500px}
...
4. In the end you should be able to see the main screen of Transkribus and log in (with the credentials you established in Step 1 of this guide) via the button on top left. 
...
## Segmenting and transcribing texts - Read (or watch) before class

### Written instructions

1. Segmentation and transcription are both covered in [How to use Transkribus in 10 Steps](https://readcoop.eu/transkribus/howto/use-transkribus-in-10-steps/) again focusing on steps 1-8 for a general overview of tools available.

2. After you are familiar with the overall process, please consult the [Transkribus Transcription Guidelines](https://readcoop.eu/transkribus/howto/transkribus-transcription-conventions/) to understand the know-how of transcription before you commence with your document. 

3. There are additional tips and tricks for special topics that apply to, for example, [Transcribing Tables](https://readcoop.eu/transkribus/howto/how-to-work-with-tables-in-transkribus/), should you run into these in your casefiles (we suggest you avoid transcribing tables in this course). 
...
...which is not covered in the 10 Steps above

https://readcoop.eu/transkribus/howto/how-to-export-documents-from-transkribus/ 

# Your task
Your group will be allocated a folder number to download from the Shared Drive: ARTS_DigitalArchives2021\Transkribus. As part of this exercise each of you will transcribe one page from the archival sources. You will work with your group in breakout rooms in Zoom but also switch to plenary sessions to get guidance on any issues. **You are strongly advised to obtain a mouse for this exercise**.
...
6. Each group member should submit one page of segmented and transcribed text to your group GoogleDrive space. Coordinate with others or select a reviewer from among the team to avoid duplicates.
